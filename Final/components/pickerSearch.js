...
export function bindPickerSearch({ modal, input, body, columnCount, load, onPending, onResults, getSort }) {
    let timer;
    let revision = 0;
    let disposed = false;
    ...
    function schedule(delay = 300) {
        ...
        timer = setTimeout(async () => {
            const isCurrent = () => !disposed && currentRevision === revision && modal.isConnected;
            try {
                ...
                console.error(error);
                pagination.error();
                showMessage("Não foi possível realizar a busca. Tente novamente.", true);
            } finally {
    ...
    const onInput = () => { pagination.reset(); schedule(); };
    function dispose() {
        if (disposed) return;
        disposed = true;
        ++revision;
    ...
    schedule(0);
    dispose.reset = () => { pagination.reset(); schedule(0); };
    return dispose;
}
