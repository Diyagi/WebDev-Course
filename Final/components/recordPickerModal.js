...
// Opens the record picker and configures search and selection.
export async function openRecordPicker({
  title,
  kicker = "Seleção",
...
async function ensureComponent() {
  ...
  if (!componentPromise) {
    componentPromise = fetch("/components/recordPickerModal.html")
      .then((response) => {
        if (!response.ok)
          throw new Error("Não foi possível carregar o seletor.");
      ...
      .then((html) => {
        ...
        modal
          .querySelector("#recordPickerSearch")
        ...
        modal
          .querySelector("#recordPickerTableHead")
          .addEventListener("click", onSortClick);
        modal
          .querySelector("#recordPickerTableBody")
...
// Builds the table headers and sorting controls.
...
// Fills the table with the available records.
function renderRecords(modalElement) {
  ...
  const query = normalize(
    modalElement.querySelector("#recordPickerSearch").value,
  );
  filteredRecords = loadRemoteRecords
    ? [...records]
    : records
        .filter((record) =>
          normalize(
            columns.map((column) => getColumnValue(column, record)).join(" "),
          ).includes(query),
        )
        .sort(compareRecords);
...
// Finds and selects the clicked record.
function onRecordClick(event) {
  const button = event.target.closest(".select-record");
  if (!button) return;
  selectRecord(
    records.find((record) => String(record.id) === button.dataset.id),
  );
}

// Returns the selected record through the callback and closes the modal.
function selectRecord(record) {
  if (!record) return;
  onSelectRecord?.(record);
  bootstrap.Modal.getInstance(
    document.querySelector("#recordPickerModal"),
  )?.hide();
}

// Compares records using the selected sort column and direction.
...
// Gets a record value for the specified column.
