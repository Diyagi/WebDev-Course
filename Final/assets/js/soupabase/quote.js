  ...
  return readPage(
    () => {
      let query = supabase
        ...
        .select(
          QUOTE_SELECT +
            (term ? ",search_client:clientid(),search_seller:userid()" : ""),
        );
      if (!options.showFinalized) query = query.is("finalized_at", null);
      if (term) {
        query = query
          .or(textSearchFilter(["name"], term), {
            referencedTable: "search_client",
          })
          .or(textSearchFilter(["full_name", "username"], term), {
            referencedTable: "search_seller",
          });
      }
      return applySearch(
        query,
        term,
        [],
        ["search_client.not.is.null", "search_seller.not.is.null"],
      );
    },
    { ...options, sort: { key: "created_at", direction: "desc" } },
    (quote) => {
      if (quote.finalized_at || options.showExpired || !quote.created_at)
        return true;
      const expiration = new Date(quote.created_at);
      expiration.setDate(
        expiration.getDate() + Number(quote.validity_days || 0),
      );
...
// Fetches the quote details by ID.
...
// Marks an open quote as finalized.
...
// Saves the quote details and its items.
...
// Deletes the items and the specified quote.
export async function deleteQuote(quoteId) {
  ...
  return await supabase.from("quote").delete().eq("id", quoteId);
}

// Reopens an expired quote and recalculates its items using current prices.
export async function reopenExpiredQuote(quoteId) {
  const { data: quote, error: quoteError } = await supabase
    .from("quote")
    .select(
      "id, created_at, validity_days, finalized_at, product_quote(id, productid, amount)",
    )
    .eq("id", quoteId)
    .single();
  ...
  if (quote.finalized_at || expiration >= new Date()) {
    return {
      ...
      error: {
        ...
        message: "O orçamento não está vencido.",
      },
    };
  }
  ...
  for (let start = 0; start < productIds.length; start += 100) {
    ...
    do {
      const result = await readPage(
        () => supabase.from("product").select("id, price").in("id", ids),
        { cursor },
      );
      if (result.error) return { data: null, error: result.error };
  ...
  const unavailableProduct = productIds.find(
    (productId) =>
      !prices.has(String(productId)) ||
      !Number.isFinite(prices.get(String(productId))),
  );
  if (unavailableProduct !== undefined) {
    return {
      ...
      error: {
        ...
        message: `Produto ${unavailableProduct} indisponível.`,
      },
    };
  }
