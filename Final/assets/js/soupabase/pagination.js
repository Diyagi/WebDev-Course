...
// Prepares a value for use in pagination filters.
function quoteValue(value) {
  return `"${String(value).replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"`;
}

// Nulls are always last; id makes every ordering unique, including tied values.
// Applies the cursor and sorting to the next page query.
export function seek(
  query,
  cursor,
  { key = "id", direction = "asc", related } = {},
) {
  const ascending = direction === "asc";
  const operator = ascending ? "gt" : "lt";
  if (related) {
    query = query
      .order(`${related.relation}(${related.column})`, {
        ascending,
        nullsFirst: false,
      })
      .order("id", { ascending });
    ...
    if (cursor.value == null)
      return query.or("cursor_present.is.null")[operator]("id", cursor.id);
    return query[operator](`cursor_after.${related.column}`, cursor.value)
      .eq(`cursor_equal.${related.column}`, cursor.value)
      .or(
        `cursor_after.not.is.null,and(cursor_equal.not.is.null,id.${operator}.${cursor.id}),cursor_present.is.null`,
      );
  }
  ...
  if (cursor.value == null)
    return query.is(key, null)[operator]("id", cursor.id);
  const value = quoteValue(cursor.value);
  return query.or(
    `${key}.${operator}.${value},and(${key}.eq.${value},id.${operator}.${cursor.id}),${key}.is.null`,
  );
}

// Fetches a page of records and calculates the next cursor.
export async function readPage(
  ...
  const size = Math.max(
    1,
    Math.min(100, Math.trunc(Number(pageSize)) || PAGE_SIZE),
  );
  const rows = [];
  let after = cursor;
  const valueOf = (row) =>
    sort.related
      ? (row[sort.related.relation]?.[sort.related.column] ?? null)
      : (row[sort.key] ?? null);
  ...
  while (rows.length <= size && isCurrent()) {
    const { data, error } = await seek(makeQuery(), after, sort).limit(
      size + 1,
    );
  ...
  return {
    ...
    error: null,
    nextCursor:
      rows.length > size ? { id: last.id, value: valueOf(last) } : null,
  };
}
