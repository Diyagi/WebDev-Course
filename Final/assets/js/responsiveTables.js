// Adapts existing and newly added tables for small screens.
export function initResponsiveTables() {
  const selector = ".table-responsive > table";

  // Adds labels and accessibility attributes to a table.
  function prepareTable(table) {
    ...
    table.classList.toggle(
      "mobile-list-sortable",
      Boolean(headerRow.querySelector("button")),
    );
    ...
    for (const mutation of mutations) {
      ...
      for (const node of mutation.addedNodes) {
        ...
        node
          ...
          .forEach((addedTable) => tables.add(addedTable));
      }
    }
