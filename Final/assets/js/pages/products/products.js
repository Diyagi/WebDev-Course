...
// Fetches products and fills the page list.
async function loadProducts(search, isCurrent, options) {
  ...
  const {
    ...
    error,
    nextCursor,
  } = await dbProduct.getProducts(search, options);
  if (!isCurrent()) return;
  ...
  products.forEach((product) => {
    ...
    row.querySelector(".delete-product").dataset.name =
      product.description ?? "este produto";
