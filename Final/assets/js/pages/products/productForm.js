...
// Opens the product category picker.
function showCategoryPicker() {
  openRecordPicker({
    ...
    onSelect: (category) => {
      ...
      validateField(checkCategory);
    },
  }).catch((error) => {
    console.error(error);
    formFeedback.showMessage(
      "danger",
      "Não foi possível abrir o seletor de categorias.",
    );
  });
}

// Updates the selected category in the form.
function setCategory(category) {
  categorySelect.value = category.id;
  document.querySelector("#productCategoryLabel").textContent =
    category.description || `Categoria #${category.id}`;
}

// Loads the product details into the form.
async function loadProduct() {
  ...
  productData = data;
  document.querySelector("#productId").textContent =
...
// Formats the entered price and updates validation.
...
// Fills the price field with the formatted value.
function setPriceValue(price) {
  const value = Number(price);
  priceInput.value = Number.isFinite(value) ? formatPrice(value) : "";
}

// Converts the entered price to a numeric value.
function getPriceValue() {
  const digits = priceInput.value.replace(/\D/g, "");
  return digits ? Number(digits) / 100 : null;
}

// Formats the price for display in the edit field.
...
function checkCategory(showErrors) {
  ...
  if (categorySelect.disabled || !categorySelect.value) {
    if (showErrors)
      formFeedback.setInvalid(categoryPicker, "Selecione uma categoria.");
    return false;
  }
...
// Validates the selected product status.
function checkStatus(showErrors) {
  formFeedback.clearValidation(statusSelect);

  if (!statusSelect.value) {
    if (showErrors)
      formFeedback.setInvalid(statusSelect, "Selecione um status.");
    return false;
  }
