...
export async function init() {
  ...
  document
    .querySelector("#addQuoteProduct")
    .addEventListener("click", showProductPicker);
...
// Loads the sellers available for the quote.
async function loadSellers() {
  ...
  if (loggedUser) {
    ...
    if (
      !sellers.some((seller) => String(seller.id) === String(currentSeller.id))
    )
  ...
  if ((editMode || !users.length) && !loggedUser) {
    ...
    document.querySelector("#quoteSellerLabel").textContent = editMode
...
// Loads the quote details into the form.
async function loadQuote() {
  ...
  quoteData = data;
  document.querySelector("#quoteId").textContent =
    `ID do Orçamento: ${data.id}`;
  const selectedClient = {
    ...data.client,
    id: data.clientid,
    name: data.client?.name || `Cliente #${data.clientid}`,
  };
  setClient(selectedClient);

  const selectedSeller =
    sellers.find((seller) => String(seller.id) === String(data.userid)) ||
    normalizeSeller({ id: data.userid, profile: data.seller });
  if (
    !sellers.some((seller) => String(seller.id) === String(selectedSeller.id))
  )
    sellers.push(selectedSeller);
  ...
  if (!loggedUser)
    disableForm(
      "Não foi possível identificar o usuário conectado para editar este orçamento.",
    );
}

// Opens the quote client picker.
  ...
  openRecordPicker({
    ...
    tableColumns: [
      ...
      { key: "name", label: "Nome" },
      {
        key: "cpf_cnpj",
        label: "CPF/CNPJ",
        value: (client) => formatDocument(client.cpf_cnpj, "Sem documento"),
      },
      {
        key: "phone",
        label: "Telefone",
        value: (client) => formatPhone(client.phone),
      },
  ...
  openRecordPicker({
    ...
    tableColumns: [
      ...
      { key: "email", label: "Email" },
      {
        key: "role",
        label: "Função",
        value: (seller) => formatRole(seller.role),
      },
    ],
    onSelect: (seller) => {
      setSeller(seller);
...
function showProductPicker() {
  ...
  openProductPicker({
    ...
    formFeedback.showMessage(
      "danger",
      "Não foi possível abrir o seletor de produtos.",
    );
  });
}

// Adds a new product row to the quote.
...
// Fills the table with the quote items.
function renderItems() {
  itemsBody.innerHTML = "";
  if (!items.length) {
    itemsBody.innerHTML =
      '<tr><td colspan="6" class="text-center text-muted py-4">Nenhum produto adicionado.</td></tr>';
    updateTotal();
    return;
  }
  ...
  items.forEach((item, index) => {
    ...
    row.querySelector(".item-value").value = Number(item.product_value).toFixed(
      2,
    );
    row.querySelector(".item-total").textContent = formatCurrency(
      item.amount * item.product_value,
...
// Updates items, totals, and validation after a change.
function onFormInput(event) {
  const row = event.target.closest("tr[data-index]");
  if (row) {
    const item = items[Number(row.dataset.index)];
    if (event.target.matches(".item-description"))
      item.description = event.target.value;
    if (event.target.matches(".item-amount"))
      item.amount =
        event.target.value === "" ? null : Number(event.target.value);
    if (event.target.matches(".item-value"))
      item.product_value =
        event.target.value === "" ? null : Number(event.target.value);
    row.querySelector(".item-total").textContent = formatCurrency(
      item.amount * item.product_value,
...
// Removes the selected item and updates the quote.
...
async function onSubmit(event) {
  ...
  formFeedback.showMessage(
    "danger",
    error.message || "Não foi possível salvar o orçamento.",
  );
...
function validateForm(showErrors = false) {
  ...
  [clientPicker, sellerPicker, validityInput].forEach((field) =>
    formFeedback.clearValidation(field),
  );
  const clientValid = Boolean(clientSelect.value) && !clientPicker.disabled;
  const sellerValid =
    Boolean(sellerSelect.value) &&
    (!sellerPicker.disabled || (editMode && Boolean(loggedUser)));
  const validityValid =
    Number.isInteger(Number(validityInput.value)) &&
    Number(validityInput.value) > 0;
  const itemsValid =
    items.length > 0 &&
    items.every(
      (item) =>
  ...
  if (showErrors && !clientValid)
    formFeedback.setInvalid(clientPicker, "Selecione um cliente.");
  if (showErrors && !sellerValid)
    formFeedback.setInvalid(sellerPicker, "Selecione um vendedor.");
  if (showErrors && !validityValid)
    formFeedback.setInvalid(
      validityInput,
      "Informe uma validade maior que zero.",
    );
  document.querySelector("#quoteItemsError").textContent =
    showErrors && !itemsValid
      ? "Adicione ao menos um produto e informe quantidade e valor válidos."
      : "";
...
// Updates the displayed quote total.
function updateTotal() {
  document.querySelector("#quoteTotal").textContent = formatCurrency(
    getTotal(),
    { coerceInvalidToZero: true },
  );
}

// Calculates the sum of the quote item values.
function getTotal() {
  return roundMoney(
    items.reduce(
      (total, item) =>
        total + (Number(item.amount) || 0) * (Number(item.product_value) || 0),
      0,
    ),
  );
}

// Updates the selected client in the form.
function setClient(client) {
  clientSelect.value = client.id;
  const label = document.querySelector("#quoteClientLabel");
  label.textContent = `${client.name || `Cliente #${client.id}`} · ${formatDocument(client.cpf_cnpj, "Sem documento")}`;
  label.classList.remove("text-muted");
}

// Updates the selected seller in the form.
...
// Standardizes seller data returned by different queries.
...
// Disables the form and displays the reason.
...
// Updates the guidance about quote finalization.
...
// Checks whether the current user can change the seller.
function canEditSeller() {
  return editMode && ["owner", "admin"].includes(loggedUser?.role);
}

// Updates the guidance about seller selection.
