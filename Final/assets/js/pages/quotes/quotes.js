...
export async function init() {
  document
    .querySelector("#quoteTableBody")
    .addEventListener("click", onTableClick);
  document
    .querySelector("#quoteTableBody")
    .addEventListener("keydown", onTableKeydown);
  quotes = [];
  await bindListSearch({
    inputId: "quoteSearch",
    tableBodyId: "quoteTableBody",
    columnCount: 8,
    load: loadQuotes,
...
// Fetches quotes and fills the page list.
...
// Fills the table with the loaded quotes.
function renderQuotes() {
  ...
  if (!visibleQuotes.length) {
    tableBody.innerHTML =
      '<tr><td colspan="8" class="text-center text-muted">Nenhum orçamento encontrado para estes filtros.</td></tr>';
    return;
  }
  ...
  visibleQuotes.forEach((quote) => {
    ...
    const reopenAction =
      isExpired(quote) && !quote.finalized_at
            ...
            <td class="text-end text-nowrap">
                <div class="dropdown quote-actions">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        <li><a class="dropdown-item print-quote" href="/views/quotes/quotePrint.html?quoteId=${encodeURIComponent(quote.id)}" target="_blank" rel="noopener"><i class="bi bi-printer me-2" aria-hidden="true"></i>Imprimir</a></li>
                        ${reopenAction}
    ...
    row.children[1].textContent =
      quote.client?.name ?? `Cliente #${quote.clientid}`;
    row.children[2].textContent =
      quote.seller?.full_name ||
      quote.seller?.username ||
      shortId(quote.userid);
    row.children[3].textContent = formatDate(quote.created_at);
    row.querySelector(".badge").textContent = status.label;
    row.children[4].querySelector("small").textContent =
      `${quote.validity_days} dia${Number(quote.validity_days) === 1 ? "" : "s"}`;
...
// Opens the quote when Enter or Space is pressed on its row.
function onTableKeydown(event) {
  if (
    (event.key === "Enter" || event.key === " ") &&
    event.target.matches(".quote-row")
  ) {
...
// Opens the selected quote details.
  ...
  if (quote)
    showQuoteDetail(quote, {
      onFinalized: (updatedQuote) => {
        ...
        document
          .querySelector("#quoteSearch")
          .dispatchEvent(new Event("input"));
      },
    }).catch(console.error);
}

// Requests confirmation and deletes the selected record.
...
// Requests confirmation to reopen and recalculate the quote.
...
// Returns the record status and its display properties.
function getStatus(quote) {
  if (quote.finalized_at)
    return { label: "Finalizado", className: "text-bg-success" };
  if (isExpired(quote))
    return { label: "Vencido", className: "text-bg-danger" };
  return { label: "Em aberto", className: "text-bg-primary" };
}

// Checks whether the quote validity period has ended.
...
// Shortens an identifier for display in the table.
