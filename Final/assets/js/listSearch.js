...
// Connects list search and filters to pagination.
export function bindListSearch({
  inputId,
  tableBodyId,
  columnCount,
  load,
  ...
  const run = async (currentRevision) => {
    ...
    try {
      const result = await load(input.value.trim(), isCurrent, {
  ...
  for (const id of filterIds)
    document
      .getElementById(id)
      .addEventListener("change", () => schedule(0, true));
